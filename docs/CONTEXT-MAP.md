...
| `src/main.py` | Entry point: orchestrate fetch → filter → notify; resolves `DISCORD_WEBHOOK_URLS` (JSON array) with `DISCORD_WEBHOOK_URL` legacy fallback | 2026-06-10 | ✅ |
...
- `src/main.py` — Loads last run timestamp, orchestrates RSS fetch → keyword filter → Discord notify, saves new timestamp; resolves webhook list from `DISCORD_WEBHOOK_URLS` (JSON array) with `DISCORD_WEBHOOK_URL` legacy fallback via `_load_webhook_urls()`
...
- `.github/workflows/monitor.yml` — Cron `0 */4 * * *`, `workflow_dispatch`, artifact-based timestamp persistence; exposes both `DISCORD_WEBHOOK_URLS` and `DISCORD_WEBHOOK_URL` env vars
